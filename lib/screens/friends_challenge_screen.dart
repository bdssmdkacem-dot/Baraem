...
class _FriendsChallengeScreenState extends State<FriendsChallengeScreen> {
  ...
  void _start() {
    ...
    if (players.isEmpty || players.length > 3) return;
  ...
  Future<void> _addPlayer() async {
    if (context.read<PlayersProvider>().players.length >= 4) return;
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            DropdownButtonFormField<String>(value: gender, decoration: const InputDecoration(labelText: 'الجنس'), items: const [DropdownMenuItem(value: 'boy', child: Text('ولد')), DropdownMenuItem(value: 'girl', child: Text('بنت'))], onChanged: (v) => setDialogState(() => gender = v ?? 'boy')),
          ]),
          actions: [
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: _engine == null ? _setup(context) : current == null ? _finishChallenge(context) : _question(current),
      ),
    );
  }
  ...
  Widget _setup(BuildContext context) {
    ...
    return Column(children: [
      ...
      Text('اختر من 1 إلى 3 لاعبين • $selectedCount/3', style: const TextStyle(fontSize: 18)),
      const SizedBox(height: 12),
      Expanded(child: players.isEmpty
          ? const Center(child: Text('أضف أول لاعب ليبدأ التحدّي'))
          : ListView(children: players.map((p) => Card(child: CheckboxListTile(
              value: _selectedPlayerIds.contains(p.id),
              onChanged: selectedCount >= 3 && !_selectedPlayerIds.contains(p.id) ? null : (value) => setState(() => value == true ? _selectedPlayerIds.add(p.id) : _selectedPlayerIds.remove(p.id)),
              title: Text(p.nickname),
              subtitle: Text('${p.age} سنة • ⭐ ${p.stars}'),
            ))).toList())),
      ...
      FilledButton.icon(onPressed: selectedCount >= 1 && selectedCount <= 3 ? _start : null, icon: const Icon(Icons.play_arrow), label: Text(selectedCount == 1 ? 'ابدأ التحدي الفردي' : 'ابدأ التحدّي')),
    ]);
  }
  ...
  Widget _question(ChallengeQuestion current) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Text('العمر ${current.player.age} سنة • الجولة ${_round + 1} • ⭐ ${current.player.stars + (_scores[current.player.id] ?? 0)}', textAlign: TextAlign.center),
      const SizedBox(height: 24),
      Text(q.question, textAlign: TextAlign.center, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w800)),
      if (q.audioAsset != null) IconButton(onPressed: () => _audioProvider?.playAsset(q.audioAsset!), icon: const Icon(Icons.volume_up_rounded)),
      const SizedBox(height: 12),
  ...
  Widget _finishChallenge(BuildContext context) {
    ...
    return Column(children: [
      ...
      const SizedBox(height: 24),
      Expanded(child: ListView.builder(itemCount: ranked.length, itemBuilder: (_, i) => Card(child: ListTile(
        leading: Text(i == 0 ? '🥇' : i == 1 ? '🥈' : '⭐', style: const TextStyle(fontSize: 26)),
        title: Text(ranked[i].nickname),
        subtitle: Text('⭐ ${ranked[i].stars} • فاز ${ranked[i].challengesWon} مرة'),
        trailing: Text('${_scores[ranked[i].id] ?? 0} نقاط'),
      )))),
